...
if [ "$BEHIND" -gt 0 ]; then
    echo "Your branch is $BEHIND commits behind origin/$BRANCH"
...
- **Commit notes:** when the grounding block carries a `commit_notes:` section (from `auto-ticket`'s fix pass, contract §3), write each of its lines into the body of the commit that holds the files it names, or into the single commit's body.
