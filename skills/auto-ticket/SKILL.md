...
user-invocable: true
allowed-tools: Read(*), Bash(git:*, gh:*), Bash(rm -f .ccmagic-ticket.md), Bash(${CLAUDE_PLUGIN_ROOT}/bin/ccm-context *), Bash(ccm-context *), Bash(${CLAUDE_PLUGIN_ROOT}/bin/ccm-ci-status *), Bash(ccm-ci-status *), Bash(${CLAUDE_PLUGIN_ROOT}/bin/ccm-pr-threads *), Bash(ccm-pr-threads *), Glob(*), Grep(*), Task(*), TodoWrite(*), Skill(*)
...
**Fix-pass git check.** A fix pass must change files and leave them uncommitted on the PR's branch. Before each fix pass (here and in Step 4b), record `git rev-parse HEAD`, `git branch --show-current`, and the tree state `git status --porcelain` plus `git diff HEAD | git hash-object --stdin`. After a `done`, **route-and-stop** (reason names the fix pass and what went wrong) when:
- the HEAD or the branch changed: "fix pass {n} ({fix_source}) committed or switched branch; it must leave changes uncommitted";
- the tree state is unchanged: "fix pass {n} ({fix_source}) reported done but changed no files".
...
(The `ccm-*` scripts are also on the Bash `PATH` while the plugin is enabled; if the `${CLAUDE_PLUGIN_ROOT}/bin/` path doesn't resolve, call them by bare name.)
...
**4b. Validate locally before trusting CI.** Run the validate step via `run_step`: `/ccmagic:validate`. `done` → continue to 4c. A `needs-human` whose `reason` starts with `failed:` means a check failed. Any other `needs-human` (including "validate produced no handshake") is not a check failure: **route-and-stop** (stage = `validate`) with its reason. On a check failure, fix the regressions in up to `max_validate_attempts` attempts (default **2**). Each attempt runs a fix pass on the work step via `run_step`, as in Step 3 items 1 and 2 (including the fix-pass git check), with `fix_source: validate`, `fix_pass: {attempt}`, and a `findings_to_fix:` section holding the validate step's `failures:` section as reported (contract §3); then runs the validate step again. A fix pass that returns `needs-human` → **route-and-stop** (stage = `validate`) with its reason. Once validate returns `done`, commit and push via `/ccmagic:push` (run this via `run_step`, with every attempt's `commit_notes:` lines combined into one section when they reported any); if push returns `needs-human`, **route-and-stop**. If validation still fails after the attempts → **route-and-stop** (reason: "local validation fails: {summary}"). Doing this locally keeps CI + bot-review round-trips rare.
...
**4c. Wait for CI and new reviews.** This pass's push(es) already happened (in 4a via `pr-feedback`, and possibly again in 4b); `H` was captured before them at the top of the pass. Now that they've landed:
  - **Wait for CI to settle** with one command, repeated only while it says so:
    ```bash
    "${CLAUDE_PLUGIN_ROOT}/bin/ccm-ci-status" {PR_NUMBER} --watch --wait-key {run_id}-pass{n}
    ```
  ...
  - Then read the review threads against `H`:
    ```bash
    "${CLAUDE_PLUGIN_ROOT}/bin/ccm-pr-threads" {PR_NUMBER} --since-id {H}
    ```
...
Post every PR comment with the body on stdin through a quoted heredoc, which passes it through unexpanded:

```bash
gh pr comment {PR_NUMBER} --body-file - <<'CCM_SUMMARY_EOF'
{the summary above, verbatim}
CCM_SUMMARY_EOF
```

Never write the summary to a temp file first: a session with narrow Bash grants refuses a write under `/tmp`, and this skill can't delete a file from the repo, so it would be left in the working tree.
...
| A fix pass on the work step (Step 3 or 4b) returns `needs-human`, does not report an item it was sent, commits or switches branch, or changes no files | route-and-stop (stage `review-ticket` or `validate`); nothing from that pass is pushed, and the parked comment lists the uncommitted files it left. |
| `validate` → `needs-human` whose reason does not start with `failed:` (a crash, no handshake) | route-and-stop (stage `validate`) without a fix pass. |
