...
2. **Rerun the corpus.** If the verifier fuzzed or enumerated, rebuild that corpus from its stated generator or rule and run it against the fix. Keep it within the scratch-program limits in `skills/review/agent-instructions.md`: a hard timeout of about 60 seconds (`timeout -k 5 60 ...`) and the smallest corpus that exercises the behavior. If the verifier's corpus does not fit, sample it down and say so in the commit body. The result is zero failures, or the first failing input.
