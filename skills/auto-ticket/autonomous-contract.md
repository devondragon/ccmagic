...
**Load-with-retry.** When the server is available by signal 2 or 3 but its tools are not yet callable, load them before use — and give the asynchronous connect real wall-clock time, because back-to-back `ToolSearch` calls return in well under a second combined and will **not** outlast a remote MCP handshake (which takes several seconds). Loop up to **5 attempts**: on each attempt run `ToolSearch` for `mcp__*linear*__get_issue` (plus the other tools the step needs); if it resolves, `transport = mcp` and stop looping; otherwise wait ~3 seconds with `Bash` (`sleep 3`) before the next attempt (≈15s worst case). If a server was registered but its tools never load after all 5 attempts, fall back to `prompt-relay` **only when ticket content was injected** (condition (c)), and note the degraded transport in the run output; if content was not injected, stop with the §7 `fetch_ticket` setup-error (never hang, never guess). When the tools are immediately callable (e.g. a laptop's always-on connector) the first attempt resolves and no wait occurs. The 5-attempt / ~3-second bounds are tunable.
