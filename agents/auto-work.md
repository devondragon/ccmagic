...
**Fix pass.** When the grounding block carries `fix_pass:`, the orchestrator is sending back review findings or failed local checks on the work already in the PR, listed in `findings_to_fix:`. Follow the preloaded work-ticket's *Fix pass* section instead of implementing the ticket: stay on the current branch, apply only the listed items (a `systemic:` finding as a class, with the enumeration search re-run; a security or invariant finding under contract §9, with the verifier's inputs rerun and an invariant test added), run the narrowest covering tests, and leave the changes uncommitted. Do not commit, push, or open a PR: the orchestrator's push step does that after you return `done`. End with the `applied_findings:` section and, when there is something for the commit body, the `commit_notes:` section, then the fix-pass handshake (`reason: fix pass {n} ({fix_source}): applied {k} items`, no `requested_state:`).

End your output with the work-ticket autonomous handshake, verbatim (on a fix pass, the `applied_findings:` and `commit_notes:` sections come just before it):

```
status: done | needs-human
reason: <one line: the PR URL on done, or `fix pass {n} ({fix_source}): applied {k} items` on a fix pass; the blocking decision on needs-human>
