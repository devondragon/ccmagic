...
| `ccm-validate` | Do the project's checks pass: runs `format`, `lint`, `types`, `test`, and `build`, one command each from a `validate_<check>` config key or detected from `package.json`, `Makefile`, Gradle, Maven, `go.mod`, `Cargo.toml`, or `pyproject.toml`, and uses each command's exit code as the verdict (`pass`, `fail`, `nothing-to-run`). `--only` runs a subset and `--list` shows the commands without running them. Each check is bounded by `validate_timeout_seconds`. A Gradle or Maven build (run through `./gradlew` or `./mvnw` when present) supplies `test` and `build` ahead of `package.json` and the `Makefile`, which still supply `format`, `lint`, and `types` |
