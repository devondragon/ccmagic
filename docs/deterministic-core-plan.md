...
- **Skills** call scripts as `"${CLAUDE_PLUGIN_ROOT}/bin/ccm-<name>"`, grant that path and the bare name in `allowed-tools` (for example `Bash(${CLAUDE_PLUGIN_ROOT}/bin/ccm-validate *), Bash(ccm-validate *)`), and say the bare name works too, because plugin `bin/` is on `PATH`. Skills must not re-derive what the script decided.
