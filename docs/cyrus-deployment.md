...
1. **Laptop simulation first, no Cyrus needed.** Run `tests/relay-smoke.sh`. It runs `claude -p` under a throwaway `HOME` with no Linear MCP, gives it the template above filled with a fake ticket, and runs `/ccmagic:auto-ticket` against the private sandbox repo `devondragon/ccmagic-relay-sandbox` (pinned to `tracker: linear` and `merge_owner: reeve`, so nothing merges). It then checks that no Linear MCP was present, the handoff file was deleted and never committed, a PR was opened, and the final output carries the relay block and a `Requested state:` line; it closes the sandbox PR afterward. It grants tools as Cyrus does, plain `Bash` included; `--narrow-bash` grants the narrow rule set from *Without plain `Bash`* above instead and also checks that no `ccm-*` call was denied. This exercises transport detection, fetch-from-handoff-file, the state-change no-ops, and final-message emission without touching a real ticket or container. It calls the model and pushes to GitHub, so it is run by hand, not in CI; `tests/relay-smoke.sh --help` has the options.
